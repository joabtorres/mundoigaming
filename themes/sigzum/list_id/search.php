...
<div class="container-fluid">
    ...
    <div class="row" id="painel_de_consulta">
        <div class="col">
            <form method="POST" action="<?= url("/list_id") ?>">
    ...
    <div class="row">
        <div class="col mb-2 mt-2">
            <section class="card border-default">
                <header class="card-header bg-default">
                    <h1 class="card-title h6 mb-1 mt-1">Total de Registros Encontrados: <?= ($listIdsTotal ?? "") ?></h1>
                </header>
                        ...
                        <tbody>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="align-middle"><?= ($id->identify ?? "") ?></td>
                                        ...
                                        <td class="align-middle table-acao text-center">
                                            <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal_view_<?= (md5($id->id) ?? "") ?>" title="Visualizar"><i class="fa fa-eye"></i></button>
                                            <a class="btn btn-primary btn-sm" href="<?= url("list_id/update/{$id->id}") ?>" title="Editar"><i class="fa fa-pencil-alt"></i></a>
                                            <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal_remove_<?= (md5($id->id) ?? "") ?>" title="Excluir"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                            <?php
                                    $qtd++;
    ...
    <!-- novo-registro -->
    <?php $this->insert("list_id/register") ?>
    ...
    if (isset($listIds) && is_array($listIds)) :
        foreach ($listIds as $id) :
            $this->insert("list_id/view", ["id" => $id]);
        endforeach;
    endif;
    ?>
    ...
    if (isset($listIds) && is_array($listIds)) :
        foreach ($listIds as $id) :
            $this->insert("list_id/remove", ["id" => $id]);
        endforeach;
    endif;
    ?>