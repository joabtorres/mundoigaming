...
<div class="container-fluid">
    <div class="row">
        <div class="col" id="pagina-header">
            <h5>Uploads por Usuários</h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= url() ?>"><i class="fa fa-tachometer-alt"></i> Inicial</a>
                    </li>
                    <li class="breadcrumb-item"><a href="<?= url("/upload/users") ?>"><i class="fas fa-tasks"></i>
                            Uploads por Usuários</a></li>
                </ol>
            </nav>
        </div>
        <!--fim pagina-header;-->
    ...
    <div class="row" id="painel_de_consulta">
        <div class="col">
            <form method="POST" action="<?= url("/upload/users") ?>">
    ...
    if (isset($users) && is_array($users)) :
        foreach ($users as $user) :
            $this->insert("upload/view_list_upload", ["user" => $user]);
        endforeach;
    endif;
    ?>
    ...
    <?php
    if (isset($users) && is_array($users) && 1 == 2) :
        foreach ($users as $user) :
            if (user()->level > 1  || $user->status == 1) :
                $this->insert("upload/remove", ["upload" => $user]);
            endif;
        endforeach;
    endif;
    ?>